...
<body>
	...
	<pre>
		[ 표현법 ]
		&lt;c:forTokens var="각값을 보관할 변수명" items="분리시키고자하는문자열" delims="구분자"&gt;
			반복적으로 실행할 문구(출력만)
		&lt;/c:forTokens&gt;
	...
	<!-- 테스트할 문자열 변수 만들기 -->
	<c:set var="foods" value="아이스크림, 라면, 초밥, 럭볶이. 치킨/피자" />
	
	<ul>
		<c:forTokens var="f" items="${ foods }" delims=",./">
			<li>${ f }</li>
		</c:forTokens>
	</ul>
	...
	<pre>
		...
		- 넘겨야할 쿼리스트링이 매우 길 경우 사용하면 편하다.
	</pre>
	
	<a href="list.do?cPage=1&num=2&name=abc&food=pizza">기존방식으로 요청 </a> <br>
