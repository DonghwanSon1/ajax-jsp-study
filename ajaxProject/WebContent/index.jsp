...
<body>
	...
	<p>
		...
		우리가 그동안 기존에 a태그를 이용해서 요청 또는 form태그를 이용한 요청 방식은 동기식 요청이었다. <br>
		...
		* 동기식 / 비동기식 요청 차이 <br>
		- 동기식 : 요청 처리 후 그에 해당하는 응답 페이지가 돌아와야만 그 다음 작업이 가능 <br>
				 ...
				 전체 페이지가 리로드됌(새로고침, 즉, 페이지가 기본적으로 한번 깜빡거리면서 넘어감) <br><br>
		
		- 비동기식 : 현재 페이지는 그대로 유지하면서 중간중간마다 추가적인 요청을 보내줄 수 있고, <br>
				   응답을 받는다고해서 다른 페이지로 넘어가지는 않음(현재 페이지 그대로 유지) <br>
				   요청을 보내놓고 그에 해당하는 응답이 돌아올때 까지 현재 페이지에서 다른 작업을 할 수 있음
			예) NAVER아이디 중복체크 기능, 댓글, 실시간검색, 검색어 자동완성 <br><br>

			* 비동기식 단점 <br>
			- 현재 페이지에 지속적으로 리소스가 쌓임 => 페이지가 현저히 느려질 수 있다. <br>
			...
			* AJAX 구현방식 : JavaScript방식 / jQuery 방식 <br>
	...
	<pre>
		...
		* 주요속성
		...
		- type/method : 요청 전송 방식 (get / post 생략시  기본값은 get) => form태그의 method속성
		- data : 요청 시 전달할 값 (*{키:벨류, 키:벨류, ...}*) => form태그 안에 input태그의 value값
		...
		* 부가적인 속성
		...
		- accept : 파라미터의 타입을 설정
		- beforSend : ajax요청을 하기 전 실행되는 이벤트
	...
	<h3>1. 버튼 클릭 시 get 방식으로 서버에 데이터 전송 및 음답</h3>
		...
		function test2(){
			...
			$.ajax({
				...
				success : function(result){
					...
					// 배열 형태로 넘겼을 때 가공된것을 눈에 보여지게 하는것 뷰단의 역할(언빡싱)
			...
			$.ajax({
				...
				success : function(result){
					...
					*/
					...
					// ArrayList로 VO객체 여러개가 묶여서 보내져왔다
