...
//		System.out.println(userPwd);
		...
		// 3) 해당 요청을 처리하는 서비스클래스의 메소드를 호출
		Member loginUser = new MemberService().loginMember(userId, userPwd);
		// 일치하는 회원이 있다면 loginUser의 필드값이 회원정보로 가득 차 있을 것
		// 일치하는 회원이 없다면 loginUser의 null이 담길것
...
//		System.out.println(loginUser);
		...
		// 스텝 3. RequestDispatcher객체로부터 forward메서드 호출
		
		// 스텝 1. 어딘가에 응답화면에 보여질 값 담기(request, session, application, page)
		
		/*
		 *  * 응답페이지에 전달할 값이 있을 경우 값을 어딘가에 담아야한다. => 어딘가의 attribute영역에 담아서 보낸다.
		 *  		(담아줄수 있는 객체가 4종류 => Servlet Scope 내장객체)
		 *  
		 *  크다
		 *  
		 *  1) application : 웹 어플리케이션 전역에서 언제나 꺼내 쓸수 있음
		 *  
		 *  2) session : 모든 jsp와 servlet에서 꺼내 쓸 수 있음
		 *  			 단, 내가 직접적으로 session객체에 담은 값을 지우기 전까지만 꺼내 쓸 수 있다.
		 *  			세션이 끊기는 경우 : 브라우저가 종료될 때 , 서버가 멈춘 경우
		 *  
		 *  3) request : 해당 request를 포워딩한 응답 jsp페이지에서만 쓸 수 있다.
		 *  				요청 페이지부터 응답페이지까지에서만 쓸 수 있다.
		 * 
		 *  4) page : 담은 값을 해당 jsp페이지에서만 쓸 수 있다. (화면이 넘어가면 담은 값이 소멸)
		 *  
		 *  작다
		 *  
		 *  => session, request가 가장많이 쓰인다.
		 *  => 공통적으로 데이터를 담고자 한다면 : XXX.setAttribute(키, 벨류);
		 *  		      데이터를 뽀고자 한다면 : XXX.getAttribute(키);
		 * 			   데이터를 지우고자 한다면 : XXX.removeAttribute(키);
		 * 
		 * 예시 )
		 * 로그인 시 : session.setAttribute("userInfo", loginUser);
		 * 로그아웃 시 : session.romoveAttribute("userInfo"); 또는 무효화시키는 메서드 사용
		 * 
		 */
		
		// 스텝 2. case1. RequestDispatcher 객체 생성(응답할 뷰화면 지정 ) => forward();
	    // 		  case2. response.sendRedirect(응답할 뷰 또는 url지정); -> 재요청
		...
		if(loginUser == null) { //로그인 실패 
			...
			// Servlet에서 JSP내장객체인 session에 접근하려면 우선적으로 session객체를 얻어와야함
			...
			*/
			...
			// url 재요청 방식 (sendRedirect방식) : 사용자에게 url을 재요청 하게함으로써 응답페이지가 보여지게끔
			// response객체를 이용
