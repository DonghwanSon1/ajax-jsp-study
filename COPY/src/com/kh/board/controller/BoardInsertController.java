...
public class BoardInsertController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(ServletFileUpload.isMultipartContent(request)) {
			...
			// 파일 업로드를 위한 라이브러리면 : cos.jar(com.oreilly.servlet의 약자)
			...
			// HttpServletRequest <-(자료형) request 
			// HttpSession <-(자료형) session 
			// ServletContext <-(자료형) application
			// / 가 WebContent를 나타냄
...
//			System.out.println(savePath);
			 ...
			 * [ 표현법 ]
			 * MultipartRequest multiRequest = new MultipartRequest(request, savePath, maxSize, 인코딩, 파일명을 수정시켜주는 객체);
			 * 
			 * 위 구문 한줄 실행만으로 첨부파일이 그대로 무조건 업로드됌!!
			 * 사용자가 올린 파일명은 그래도 해당 폴더에 업로드하지 않는것이 일반적이기 때문에
			 * 파일명을 수정시켜주는 객체를 생성
			 * 
			 * Q) 파일명을 수정하는 이유는?
			 * A) 같은 파일명이 있을 경우를 대비해서,
			 *    파일명에 한글/ 특수문자/ 띄어쓰기가 포함된 경우 서버에 따라 문제가 발생할 수도 있기 때문에
			 *    
			 ...
			 *    
			 * 우리 입맛대로 파일명을 수정해서 절대로 파일명이 안겹치게끔 rename을 해보자
