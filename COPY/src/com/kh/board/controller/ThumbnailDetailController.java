...
public class ThumbnailDetailController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 2. 1번이 성공 => Board에서 조회 요청, Attachment에서도 조회 요청
		if(result > 0) {
			
			// BOARD테이블에 조회 요청
			// 기존에 우리가 만들어놨던 selectBoard메소드를 호출해서 재활용하려고 하니깐
			// 문제는 일반게시판의 경우 카테고리가 있어서 조인을 했을때 카테고리가 null인애가 없었다.
			// 사진게시글 경우 카테고리가 전부 null이기 때문에 innerjoin으로는 조회가 불가능하다.
			// => 카테고리 컬럼을 기준으로 일치하는 컬럼도, 일치하지 않는 컬럼도 전부 가져오려면
			// 기존 조인에서 outerjoin으로 바꿔야 한다.(왼쪽 테이블을 기준으로)
